...
import { PhotoStore, encodePhoto } from '../src/photos';
...
const items:Seed[]=[
  {title:'Black leather biker jacket',category:'CLOTHES',condition:'GOOD',price:150,swapValue:150,photo:'photo-1551028719-00167b16eac5',owner:0},
  {title:'Tan Nike sneakers',category:'SHOES',condition:'GOOD',price:220,swapValue:220,photo:'photo-1549298916-b41d501d3772',owner:1},
  {title:'Navy everyday backpack',category:'BAGS',condition:'LIKE_NEW',price:120,swapValue:120,photo:'photo-1553062407-98eeb64c6a62',owner:2},
  ...
  {title:'Black quilted crossbody bag',category:'BAGS',condition:'GOOD',price:null,swapValue:200,photo:'photo-1548036328-c9fa89d128fa',owner:4,description:'Roomy and strong. Looking to swap for shoes or a small speaker.'},
  {title:'Green velvet 3-seater sofa',category:'FURNITURE',condition:'GOOD',price:2500,swapValue:null,photo:'photo-1555041469-a586c61ea9bc',owner:5,description:'Moving out. Buyer arranges transport (trotro will not do 😅).'},
  {title:'Painted wooden stool',category:'FURNITURE',condition:'LIKE_NEW',price:350,swapValue:350,photo:'photo-1503602642458-232111445657',owner:1},
  ...
  {title:'White sports watch',category:'OTHER',condition:'LIKE_NEW',price:450,swapValue:450,photo:'photo-1523275335684-37898b6baf30',owner:5},
...
async function main() {
  ...
  // With Cloudflare R2 configured, demo photos are copied into our own bucket (WebP + preview) instead of hot-linking Unsplash.
  const store=new PhotoStore();
  const photoFor=async(photo:string)=>{
    const source=`https://images.unsplash.com/${photo}?auto=format&fit=crop&w=1400&q=85`;
    if(store.backend!=='r2') return source.replace('w=1400&q=85','w=900&q=75');
    const response=await fetch(source);
    if(!response.ok) throw new Error(`Could not download demo photo ${photo}: HTTP ${response.status}`);
    const {full,thumb}=await encodePhoto(Buffer.from(await response.arrayBuffer()));
    return store.save(full,thumb,`seed-${photo}`);
  };
  for(const [i,{photo,owner,description,...item}] of items.entries()) {
    const seller=sellers[owner];
    const data={...item,ownerId:userIds[owner],sell:item.price!=null,swap:item.swapValue!=null,description:description??'Well loved, well looked after. Ready for a new home in Accra. Message me and let’s make a deal!',photos:[await photoFor(photo)],area:seller.area,latitude:seller.latitude,longitude:seller.longitude};
  ...
  console.log(`Seeded ${sellers.length} sellers and ${items.length} items around Accra (photos: ${store.backend==='r2'?'Cloudflare R2':'Unsplash links'}).`);
